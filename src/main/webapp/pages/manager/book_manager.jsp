        ...
        $("#pn_input").keydown(function (e) {
          if (e.keyCode === 13)
            window.location.href =
              "BookServlet?method=getAllManagerAfter&pageNow=" +
              $("#pn_input").val();
        });
      });
    </script>
    ...
    <div id="main">
      <table>
        ...
        <c:forEach items="${page.pageData}" var="book">
          <tr>
            ...
            <td>
              <a
                href="BookServlet?method=getBook&pageNow=${page.pageNow}&id=${book.id}"
                >修改</a
              >
            </td>
      ...
      <div id="page_nav">
        ...
        <c:if test="${page.hasPrevious}">
          <a
            href="BookServlet?method=getAllManagerAfter&pageNow=${page.pageNow-1}"
            >上一页</a
          >
        </c:if>
        <c:choose>
        ...
        <c:forEach begin="${begin}" end="${end}" var="pnum">
          <c:if test="${pnum == page.pageNow}"> 【${page.pageNow}】 </c:if>
          <c:if test="${pnum != page.pageNow}">
            <a href="BookServlet?method=getAllManagerAfter&pageNow=${pnum}"
              >${pnum}</a
            >
          </c:if>
        </c:forEach>
        ...
        <a
          href="BookServlet?method=getAllManagerAfter&pageNow=${page.totalPages}"
          >末页</a
        >
        共${page.totalPages}页，${page.totalCount}条记录 到第<input
          value="${page.pageNow}"
          name="pageNow"
          id="pn_input"
        />页
