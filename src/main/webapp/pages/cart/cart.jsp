				...
				<div id="main">
					...
					<c:if test="${!empty sessionScope.items}">
						<table>
							...
							<c:forEach items="${sessionScope.items}" var="cartItem">
								<tr>
									<td>${cartItem.name}</td>
									<td>
										<input class="count" style="width: 40px" name="count" value="${cartItem.count}"
											updataid="${cartItem.id}">
									</td>
									<td>${cartItem.price}</td>
									<td id="price_${cartItem.id}">${cartItem.totalPrice}</td>
									<td><a href="CartServlet?method=deleteItem&id=${cartItem.id}">删除</a></td>
								</tr>
							</c:forEach>
						</table>
